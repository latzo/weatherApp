...
type CityFormGroupContent = {
  ...
  lat: FormControl<ICity['lat']>;
  lng: FormControl<ICity['lng']>;
};
  ...
  createCityFormGroup(city: CityFormGroupInput = { id: null }): CityFormGroup {
    ...
    return new FormGroup<CityFormGroupContent>({
      ...
      name: new FormControl(
        { value: cityRawValue.name, disabled: false },
        {
          nonNullable: true,
          validators: [Validators.required],
        },
      ),
      lat: new FormControl(cityRawValue.lat),
      lng: new FormControl(cityRawValue.lng),
    });
  }
